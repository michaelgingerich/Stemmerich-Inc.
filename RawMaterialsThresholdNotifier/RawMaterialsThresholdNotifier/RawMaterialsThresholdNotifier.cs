...
namespace RawMaterialsThresholdNotifier
{
    public partial class rawMaterialsThresholdNotifier : Form
    {
        ...
        RawMaterialsSelection rawMaterialsSelectionInstance;
        ...
        private void rawMaterialsThresholdNotifier_FormClosing(object sender, FormClosingEventArgs e)
        {
        ...
        {
            rawMaterialsSelectionInstance = new RawMaterialsSelection();
        ...
        void sendReport()
        {
            //Check that at least one e-mail is checked.
            Boolean noActiveEmails = false;
            foreach (EmailAddress emailAddressListElement in EmailAddress.emailAddressList)
            {
                if (emailAddressListElement.getEmailAddressIsChecked() == false)
                {
                    noActiveEmails = true;
                }
            }
            if(thresholdsReached && !noActiveEmails)
            {
                ...
                {
                    MessageBox.Show("There has been an issue sending a notification e-mail, because there is no internet connection.", "Internet Connection");
                }
            }
        }
